...
app.get("/available-times/:date", (req, res) => {
    ...
    // simple in-memory cache per date to reduce DB queries under load
    if (!global.__available_times_cache) global.__available_times_cache = {};
    const cache = global.__available_times_cache;
    const ttl = parseInt(process.env.AVAILABLE_TIMES_CACHE_MS || String(30 * 1000), 10); // default 30s
    if (cache[date] && (Date.now() - cache[date].ts) < ttl) {
        // cache hit
        try { metrics.increment('available_times_cache_hit'); } catch (e) {}
    ...
    db.all(
        ...
        (err, rows) => {
            ...
            try { metrics.increment('available_times_db'); } catch (e) {}
...
app.get("/google-reviews", async (req, res) => {
    ...
    if (
        ...
        metrics.increment('google_reviews_cache_hit');
    ...
    try {
        metrics.increment('google_reviews_external_call');
        ...
        metrics.increment('google_reviews_cache_fill');

        return res.json(data);
    } catch (error) {
        metrics.increment('google_reviews_external_error');
...
// Exposer métriques en mémoire pour debug local
app.get('/_metrics', (req, res) => {
    try {
        return res.json(metrics.getAll());
    } catch (e) {
        return res.status(500).json({ error: String(e && e.message ? e.message : e) });
    }
});

// Start auto-persist of metrics to backups/metrics.json
try {
    metrics.startAutoPersist(Number(process.env.METRICS_PERSIST_MS || 5000));
} catch (e) {
    console.warn('Unable to start metrics auto-persist:', e && e.message);
}

// Ensure metrics are persisted on shutdown
const gracefulPersist = () => {
    try { metrics.persist(); } catch (e) {}
};
process.on('SIGINT', () => { gracefulPersist(); process.exit(0); });
process.on('SIGTERM', () => { gracefulPersist(); process.exit(0); });
process.on('exit', () => { gracefulPersist(); });
