...
if (!global.__booking_cooldown) global.__booking_cooldown = {};
const bookingCooldown = global.__booking_cooldown;
...
module.exports = async (req, res) => {
  ...
  // anti-spam: limit booking per email (default 5 minutes)
  const now = Date.now();
  const cooldownMs = parseInt(process.env.BOOKING_COOLDOWN_MS || String(5 * 60 * 1000), 10);
  const emailKey = String(email).toLowerCase();
  if (bookingCooldown[emailKey] && (now - bookingCooldown[emailKey]) < cooldownMs) {
    metrics.increment('booking_rate_limited');
    return res.status(429).json({ error: 'Too many booking requests for this email. Please wait.' });
  }
  bookingCooldown[emailKey] = now;
  metrics.increment('booking_requests');
  ...
  if (NOTIFY_EMAIL && SMTP_HOST && process.env.SMTP_USER && process.env.SMTP_PASS) {
    try {
      ...
      metrics.increment('booking_email_errors');
