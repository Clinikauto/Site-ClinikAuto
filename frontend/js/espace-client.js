...
function syncClientAppData(client) {
  if (!client || typeof AppData === 'undefined') {
    return;
  }
  const vehicles = Array.isArray(client.vehicules) ? client.vehicules : [];
  const primaryVehicle = vehicles[0] || {};

  AppData.save('nom', client.nom || '');
  AppData.save('prenom', client.prenom || '');
  AppData.save('email', client.email || '');
  AppData.save('telephone', client.tel || '');
  AppData.save('immatriculation', primaryVehicle.immat || '');
}

function prepareGestionNavigation(target) {
  if (currentClient) {
    syncClientAppData(currentClient);
  }
  if (typeof AppData !== 'undefined' && target === 'contact') {
    const selectedService = AppData.get('selectedService');
    const selectedVehicle = AppData.get('selectedVehicle');
    const selectedDate = AppData.get('selectedDate');

    const lines = [];
    if (selectedService) lines.push('Service: ' + selectedService);
    if (selectedVehicle && selectedVehicle.title) lines.push('Vehicule occasion: ' + selectedVehicle.title);
    if (selectedDate) lines.push('Date souhaitee: ' + selectedDate);

    if (lines.length) {
      AppData.save('contactMessageHint', lines.join(' | '));
    }
  }
  return true;
}
...
function afficherVehicules(client) {
  const container = document.getElementById('dashVehiculesBody');
  if (!container) {
    return;
  }
  const vehicules = Array.isArray(client.vehicules) ? client.vehicules : [];
  if (vehicules.length === 0) {
    container.innerHTML = '<tr><td colspan="7" class="ec-empty-note ec-empty-note--cell">Aucun véhicule enregistré.</td></tr>';
    return;
  }
  container.innerHTML = vehicules.map((v, index) => {
    const vehicleId = escapeHtml(v.id || ('veh-' + index));
    const km = Number(v.km);
    const kmLabel = Number.isFinite(km) && km > 0 ? km.toLocaleString('fr-FR') + ' km' : '—';
    return `
      <tr>
        <td>${escapeHtml(v.immat || '—')}</td>
        <td>${escapeHtml(v.marque || '—')}</td>
        <td>${escapeHtml(v.modele || '—')}</td>
        <td>${escapeHtml(v.annee || '—')}</td>
        <td>${escapeHtml(v.carburant || '—')}</td>
        <td>${escapeHtml(kmLabel)}</td>
        <td>
          <div class="ec-veh-row-actions">
            <button class="ec-veh-btn ec-veh-btn--edit" onclick="ouvrirModalVehicule('${vehicleId}')">
              <i class="fa-solid fa-pen"></i> Modifier
            </button>
            ${vehicules.length > 1 ? `
            <button class="ec-veh-btn ec-veh-btn--del" onclick="supprimerVehicule('${vehicleId}')">
...
function afficherInfos(client) {
  ...
  document.getElementById('infosDisplay').innerHTML = `
    ${isIncomplete ? `<div class="notice-amber"><i class="fa-solid fa-triangle-exclamation"></i> <strong>Fiche incomplète</strong> — Merci de compléter : ${missingFields.join(', ')}</div>` : ''}
    <div class="ec-info-item"><span>Nom</span><strong>${escapeHtml(client.nom) || '<em class="text-danger">Non renseigné</em>'}</strong></div>
    <div class="ec-info-item"><span>Prénom</span><strong>${escapeHtml(client.prenom) || '<em class="text-danger">Non renseigné</em>'}</strong></div>
    <div class="ec-info-item"><span>Portable</span><strong>${escapeHtml(client.tel) || '<em class="text-danger">Non renseigné</em>'}</strong></div>
    <div class="ec-info-item"><span>Email</span><strong>${escapeHtml(client.email) || '<em class="text-danger">Non renseigné</em>'}</strong></div>
    <div class="ec-info-item ec-info-item--full"><span>Adresse</span><strong>${escapeHtml(client.adresse) || '<em class="text-danger">Non renseigné</em>'}</strong></div>
  `;
}
...
function sauvegarderFicheClient() {
  const map = [
    { key: 'nom', id: 'eNom' },
    { key: 'prenom', id: 'ePrenom' },
    { key: 'email', id: 'eEmail' },
    { key: 'telephone', id: 'eTel' }
  ];

  map.forEach(({ key, id }) => {
    const valeur = (document.getElementById(id) && document.getElementById(id).value) || '';
...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  if (btnEnregistrer) {
    btnEnregistrer.addEventListener('click', sauvegarderFicheClient);
  }
